...
void expandKey(unsigned char *key, int key_size, unsigned char *expanded_key, int expanded_key_size){
	...
	while (current_size_of_key < expanded_key_size){ 
		for(int i = 0; i < 4; ++i){
			t[i] = expanded_key[(current_size_of_key-4) + i]; 
		}
		if(current_size_of_key % key_size == 0){ 
			key_schedule_core(t, rcon_i);
			rcon_i++;
		}
		for(int i = 0; i < 4; ++i){
...
int main(){
	...
	for (int i = 0; i < 11;i++){
		for (int j = 0;j < 16;j++){
			printf("%02x ",expanded_key[i*10+j]);
		}
		cout << endl;
	}
	return 0;
